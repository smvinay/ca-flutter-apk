// lib/task_management/serviceList.dart
// Updated: Sliding card-type control kept as-is; added swipe-on-list to change card type
// - Default shows status + assignee row (with toggle icon)
// - Toggle (icon) now shows/hides a combined search input + priority row
// - Priority options are clickable only (no sliding to change them)
// - Sliding left/right on the list container switches the card-type (All / Task / Service)
// Card visuals/content remain unchanged.

import 'dart:async';
import 'dart:convert';
import 'dart:math';

// import 'package:ca_desk/service_request/service_details_old.dart';
import 'package:ca_desk/service_request/serviceView.dart';
...
class ServiceListPage extends StatefulWidget {
  ...
  const ServiceListPage({Key? key, this.onPageRequested}) : super(key: key);
...
class _ServiceListPageState extends State<ServiceListPage>
  ...
  bool _showSearch =
      false; // not the toggle-search; kept for header search if needed
  String _searchQuery = '';
  String _userType = '';
  ...
  static const String _cacheKeyServices = 'cache_service_list';
  static const String _cacheKeyServicesTs = 'cache_service_list_ts';
  ...
  final GlobalKey _ridKey = GlobalKey();
  OverlayEntry? _coachOverlay;
  bool _coachShowing = false;
  static const String _prefsCoachKey = 'seen_service_card_tip';

  late final AnimationController _ridPulseController;
  late final Animation<double> _ridPulseAnim;
  ...
  void initState() {
    ...
    // 1) Initialize entrance animation
    ...
    // 2) Initialize RID pulse animation BEFORE showing the coach mark
    _ridPulseController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 900),
    );
    _ridPulseAnim = Tween<double>(begin: 1.0, end: 1.06).animate(
      CurvedAnimation(parent: _ridPulseController, curve: Curves.easeInOut),
    );

    // 3) Perform async work (fetch services) and then show coach mark if needed.
    //    Use a separate async helper to avoid making initState() async.
    _initAsync();
  }

  Future<void> _initAsync() async {
    try {
      // Wait for services to be fetched so the first card / RID exists in the tree.
      await _fetchServices();
    } catch (e, st) {
      // Optional: log or handle fetch errors. Do not crash.
      debugPrint('Error fetching services in init: $e\n$st');
    }

    // After services are loaded and widgets built, schedule the coach mark.
  ...
  Future<void> _showCoachMarkIfNeeded() async {
    ...
    final seen = prefs.getBool(_prefsCoachKey) ?? false;
    if (seen) return;
  ...
  void _showCoachMark() async {
    ...
    _coachOverlay = OverlayEntry(
      builder: (context) {
        ...
        return Stack(
          children: [
            // Optional scrim (very subtle)
            Positioned.fill(
              child: GestureDetector(
                onTap: () => _removeCoachMark(saveSeen: true),
                child: Container(color: Colors.black.withOpacity(0.2)),
              ),
            ),

            // Pop notification bar (centered top)
            Positioned(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: const [
                              ...
                              SizedBox(height: 4),
                              Text(
                                'Tap the RID ID to see full details and actions for this request.',
                                style: TextStyle(
                                  color: Colors.black87,
                                  fontSize: 13,
                                ),
                        ...
                        ElevatedButton(
                          onPressed: () => _removeCoachMark(saveSeen: true),
                          ...
                          child: const Text(
                            'Got it',
                            style: TextStyle(color: Color(0xFF4E7BE7), fontWeight: FontWeight.w700),
                          ),
                        ),
                      ],
                    ),
    ...
    overlay.insert(_coachOverlay!);
    _coachShowing = true;
    _ridPulseController.repeat(reverse: true);
  }


  void _removeCoachMark({bool saveSeen = false}) async {
    if (!_coachShowing) return;
    ...
    try {
      _ridPulseController.stop();
    } catch (_) {}

    if (saveSeen) {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_prefsCoachKey, true);
    }
  }
  ...
  Future<String> _buildApiUrl() async {
    ...
    return '$domain$slug/serviceordersListApi?id=$id&type=$type&slug=$slug';
  }
  ...
  void _showTopToast(String message) {
    ...
    final sb = SnackBar(
      ...
      action: SnackBarAction(
        ...
        onPressed: () => _fetchServices(showLoading: true, forceRefresh: true),
      ),
    );
                ...
                Expanded(
                  child:
                      _loading
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ElevatedButton.icon(
                                    onPressed:
                                        () =>
                                            _fetchServices(forceRefresh: true),
                                    icon: Icon(
                                      Icons.refresh,
                                      size: _s(18, scale),
                                    ),
                          ...
                          : RefreshIndicator(
                            onRefresh: () => _fetchServices(forceRefresh: true),
                            child: AnimatedSwitcher(
                              duration: Duration(
                                milliseconds: (360 * scale).round(),
                              ),
  ...
  Widget _serviceCard(
    ...
    final bool chargeable =
        (chargeRaw is num)
            ? (chargeRaw == 1)
            : (chargeRaw is String)
            ? (chargeRaw.toLowerCase() == 'yes' ||
                chargeRaw == '1' ||
                chargeRaw.toLowerCase() == 'true')
            : (chargeRaw == true);
    ...
    if (rawAssigned is List) {
      assigned =
          rawAssigned.map<Map<String, String>>((e) {
            if (e is Map) {
              return {
        ...
        child: InkWell(
          ...
          onTap: () {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => serviceDetailes(ticketId: id, rid: rid),
              ),
            );
          },
                        ...
                        children: [
                            ...
                            children: [
                              ...
                              InkWell(
                                ...
                                child: ScaleTransition(
                                  scale:
                                  (isFirst && _coachShowing)
                                      ? _ridPulseAnim
                                      : const AlwaysStoppedAnimation(
                                    1.0,
                                  ),
                                  child: Container(
                                    key: isFirst ? _ridKey : null,
                                    padding: EdgeInsets.symmetric(
                                      horizontal: _s(8, scale),
                                      vertical: _s(5, scale),
                                    ),
                              ...
                              SizedBox(width: _s(4, scale)),
                              if (((s['Type']).toString().toLowerCase())
                                  .contains('service'))
                                ChargeableToggle(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              // Add space between both sections
                              SizedBox(width: _s(16, scale)),
  ...
  Widget _typeIcon(String type) {
    ...
    if (t.contains('task')) {
      return Icon(Icons.list_alt_rounded, size: 18, color: _typeColor(type));
    } else if (t.contains('service') || t.contains('service request')) {
      return Icon(Icons.receipt, size: 18, color: _typeColor(type));
    } else if (t.contains('invoice')) {
      return Icon(Icons.receipt_long, size: 18, color: _typeColor(type));
    } else {
...
// Always-visible search header
  Widget _smallHeader(double scale) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: _s(12.0, scale), vertical: _s(5, scale)),
      child: Row(
        children: [
          // Search input
          Expanded(
  ...
  // Combined status + assignee row (single row; both open bottom-sheet selectors)
...
/// Triangle arrow painter used by coach mark (keeps previous behavior but supports flip)
class _TrianglePainter extends CustomPainter {
  final Color color;
  final bool invert; // if true, arrow points downwards
  ...
  bool shouldRepaint(covariant _TrianglePainter oldDelegate) {
    return oldDelegate.color != color || oldDelegate.invert != invert;
  }
}
...
class _ChargeableToggleState extends State<ChargeableToggle> {
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor =
        _value ? Colors.green.shade50 : Colors.grey.shade100;
    final borderColor =
        _value ? Colors.green.withOpacity(0.25) : Colors.grey.shade200;
            ...
            AnimatedSwitcher(
              ...
              child:
                  _loading
                      ...
                      : Container(
                        key: ValueKey(
                          'circle_${_value ? 'checked' : 'unchecked'}',
                        ),
                        width: circleSize,
                        height: circleSize,
                        decoration: BoxDecoration(
